...
public class NatsObjStore : INatsObjStore
{
    ...
    private readonly NatsObjConfig _config;
    ...
    {
        using var activity = ObjTelemetry.StartObjectOp(connection: _context.Connection, _config, operation: "get", key: key);
    ...
    {
        using var activity = ObjTelemetry.StartObjectOp(connection: _context.Connection, _config, operation: "put", key: meta.Name);
        try
        {
    ...
    {
        using var activity = ObjTelemetry.StartObjectOp(connection: _context.Connection, _config, operation: "put_metadata", key: key);
        try
        {
            ValidateObjectName(meta.Name);
    ...
    {
        using var activity = ObjTelemetry.StartObjectOp(connection: _context.Connection, _config, operation: "add_link", key: target.Name);
        try
        {
            ...
            {
                Name = link, Bucket = Bucket, Nuid = NewNuid(), Options = new MetaDataOptions { Link = new NatsObjLink { Name = target.Name, Bucket = target.Bucket, }, },
            };
    ...
    public async ValueTask<ObjectMetadata> AddBucketLinkAsync(string link, INatsObjStore target, CancellationToken cancellationToken = default)
    {
        ...
        var info = new ObjectMetadata
        {
            Name = link, Bucket = Bucket, Nuid = NewNuid(), Options = new MetaDataOptions { Link = new NatsObjLink { Name = link, Bucket = target.Bucket, }, },
        };
    ...
    {
        using var activity = ObjTelemetry.StartSealOp(connection: _context.Connection, storeCfg: _config);
        try
        {
    ...
    {
        using var activity = ObjTelemetry.StartObjectOp(connection: _context.Connection, _config, operation: "get_info", key: key);
        try
        {
            ValidateObjectName(key);
    ...
    public IAsyncEnumerable<ObjectMetadata> ListAsync(NatsObjListOpts? opts = default, CancellationToken cancellationToken = default)
    {
        ...
        var watchOpts = new NatsObjWatchOpts { InitialSetOnly = true, UpdatesOnly = false, IgnoreDeletes = !opts.ShowDeleted, };
    ...
    private string NewNuid() => string.Create(length: 22, state: null as object, static (rawString, _) =>
    {
        if (!NuidWriter.TryWriteNuid(rawString))
