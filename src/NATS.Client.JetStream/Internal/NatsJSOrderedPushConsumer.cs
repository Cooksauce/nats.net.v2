...
internal class NatsJSOrderedPushConsumerSub<T> : NatsSubBase
{
    ...
    protected override async ValueTask ReceiveInternalAsync(
        ...
        Activity? activity)
    {
        var msg = new NatsJSMsg<T>(NatsMsg<T>.Build(subject, replyTo, headersBuffer, payloadBuffer, _nats, _headerParser, _serializer, activity), _context);
    ...
    private void OnConnectionOpened(object? sender, string e) =>
        _commands.TryWrite(new NatsJSOrderedPushConsumerMsg<T> { Command = NatsJSOrderedPushConsumerCommand.Ready });
}
